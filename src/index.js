...
async function pape(){
    ...
    tTypeSlipcase.calculateBoardLines(width, height, depth, materialThickness).forEach(line => {
        let lineColor
        if (line.color === 'black') { lineColor = { color: cmyk(0, 0, 0, 1) } };
        if (line.color === 'red') { lineColor = { color: cmyk(0, 1, 1, 0) } };
        page.drawLine({
    ...
    page.drawText(`${fileName}_papes_cirtnis: W${roundHalf(ptToMm(width))}_H${roundHalf(ptToMm(height))}_D${roundHalf(ptToMm(depth))}mm | Board: ${ptToMm(materialThickness)}mm | Dimensions: ${roundHalf(stampWidth)}x${roundHalf(stampHeight)}mm`, {
        x: 0,
        y: documentHeight - 80,
        size: 14,
        font: timesRomanFont,
...
async function parvalks() {
    ...
    hTypeSlipcase.calculateCasing(width, height, depth, materialThickness).forEach(line => {
        let lineColor
        if (line.color === 'black') { lineColor = { color: cmyk(0, 0, 0, 1) } };
        if (line.color === 'red') { lineColor = { color: cmyk(0, 1, 1, 0) } };
        page.drawLine({
    ...
    page.drawText(`${fileName}_parvalka_cirtnis: W${roundHalf(ptToMm(width))}_H${roundHalf(ptToMm(height))}_D${roundHalf(ptToMm(depth))}mm | Board: ${ptToMm(materialThickness)}mm | Dimensions: ${roundHalf(stampWidth)}x${roundHalf(stampHeight)}mm`, {
        x: 0,
        y: documentHeight - 80,
        size: 14,
        font: timesRomanFont,
